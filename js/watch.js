  ...
  if (result != null) {
      ...
      function btn_click_set(canvas_num) {
        $("#canvas_btn" + canvas_num).click(function () {
          if (canvas_flag[canvas_num]) {
            ...
            canvas_height[canvas_num] = $(
              "canvas#myLineChart" + canvas_num
            ).height();
            $.Deferred(function (deferredAnim) {
              deferredAnim.then(function () {
                $("#myLineChart" + canvas_num).animate(
      ...
      if (view_count.match(/回視聴/)) {
        ...
        $("#canvas_btn_del").click(function () {
          if ($(".youtube_live_box").html() != null) {
            ...
            config4 = config_reset("コメント");
            window.myLineChart4 = new Chart(
              document.getElementById("myLineChart4"),
              config4
            );
            window.myLineChart5.destroy();
            config5 = config_reset("スパチャ");
            window.myLineChart5 = new Chart(
              document.getElementById("myLineChart5"),
              config5
            );
          }
        });
    ...
    function canvas_update(canvas_id, config, myLineChart, value) {
      if (canvas_type["set"][canvas_id] == 1) {
        ...
        config.data.datasets.forEach(function (dataset) {
          while (
            dataset.data.length > get_chart_time(canvas_type["num"][canvas_id])
          ) {
            dataset.data.shift();
        ...
        config.data.datasets.forEach(function (dataset) {
          if (
            dataset.data.length >
              get_chart_time(canvas_type["num"][canvas_id]) &&
            ...
            canvas_wait["max"][canvas_id] =
              canvas_wait["max"][canvas_id] + canvas_wait["max"][canvas_id];
          }
        });
      } else {
        config.data.labels.push(String(Hour + ":" + Min + ":" + Sec));
    ...
    if (!view_count.match(/回視聴/)) {
      ...
      if (view_count_plus_list[view_count_plus_num] != null) {
        var view_count_plus =
          result - view_count_plus_list[view_count_plus_num];
...
function comment_get() {
  ...
  i = -1;
  while (true) {
    last_message_id = $("#chatframe")
      .contents()
      .find("yt-live-chat-text-message-renderer")
      .eq(i)
      .attr("id");
    ...
    message_count++;
    i--;
    moderator_id = $("#chatframe")
      .contents()
      .find("#author-name.moderator.yt-live-chat-author-chip")
      .eq(-1)
      .parent()
      .parent()
      .parent()
    ...
    if (menu_set[2]) {
      if (moderator_id == null) {
        ...
        moderator_message = $("#chatframe")
          ...
          .find("#message")
          .text();
        $("#youtube_moderator_message").prepend(
          '<div id="youtube_moderator_message_box" style="font-size: 13px;border-radius: 50%;">' +
            '<img class="img" alt="" height="24" width="24" src="' +
            moderator_img +
            '">' +
            '<span class="moderator_time">' +
            moderator_time +
            "</span>" +
            '<span class="moderator_name">' +
            moderator_name +
            "</span>" +
            '<p class="moderator_message">' +
            moderator_message +
            "</p>" +
            "</div>"
        );
  ...
  i = -1;
  while (true) {
    last_superchat = $("#chatframe")
      .contents()
      .find("div#purchase-amount")
      .eq(i)
      .find("yt-formatted-string")
      .html();
    last_superchat_id = $("#chatframe")
      .contents()
      .find("yt-live-chat-paid-message-renderer")
      .eq(i)
      .attr("id");
    ...
    superchat_count =
      Math.round(calc_superchat_jpy(currency, amount) * 100) / 100 +
      superchat_count;
    i--;
  }
  last_get_superchat_id = $("#chatframe")
    .contents()
    .find("yt-live-chat-paid-message-renderer")
    .eq(-1)
    .attr("id");
...
function comment_view() {
  if (menu_set[3]) {
    ...
    if ($("div[time='" + max_time + "']").attr("title") == null) {
      ...
      $(".chapter-title").find("p").text() !=
      $("div[time='" + max_time + "']").attr("title")
    ) {
      $(".chapter-title").remove();
      if (max_time > 0) {
        $(
        ...
        comment
          ...
          .each((i, element) => {
            ...
            if (comment_str.html() != null) {
              ...
              left = left + "%";
              $(".ytp-chapters-container").append(
                '<div class="chapter" title="' +
                  comment_str.next().text() +
                  '" time="' +
                  time +
                  '" style="left:' +
                  left +
      ...
      comments.each((i, element) => {
        ...
        if (
          ...
          if (author.find("#import_btn").html() == null) {
            author.append(
              '<a class="yt-simple-endpoint style-scope yt-formatted-string" id="import_btn">インポート</a>'
            );
